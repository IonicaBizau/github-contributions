...
var GenerateRepository = module.exports = function (options, done, progress) {
    ...
    var repoPath = Path.resolve(__dirname + "/../public/repos/" + Math.random().toString(36).substring(3))
     ...
     * @return {undefined}
     */
    function dummyCommit(message, callback) {
    ...
    function lastCommit() {
        ...
        , function (err) {
            ...
            repo.add(function (err) {
                if (err) { return callback(err); }
                repo.commit("Added the title and description (/cc. @IonicaBizau)", callback);
                done();
            });
        });
    }
